...
if test $DISTRO = "RHEL-7" -o $DISTRO = "all"; then

	REPODIR=/mnt/sf/repositories/RHEL-7

	rm -vrf $REPODIR
	mkdir -p $REPODIR

	wget --no-parent -m http://download.opensuse.org/repositories/home:/wolframe_user/RedHat_RHEL-7/
	cd $BASE/download.opensuse.org/repositories/home:/wolframe_user/RedHat_RHEL-7/

	sed 's|http://download.opensuse.org/repositories/home:/wolframe_user/|http://sourceforge.net/projects/wolframe/files/repositories/|g' \
		home:wolframe_user.repo | \
	sed 's|\[home_wolframe_user\]|[Wolframe]|g' | \
	sed 's|wolframe_user.s Home Project|Wolframe Project|g' | \
	sed 's|RedHat_RHEL-7|RHEL-7|g' \
		> $REPODIR/wolframe.repo
