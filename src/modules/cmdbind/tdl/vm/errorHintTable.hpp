...
public:
	...
	typedef std::vector<Hint> HintList;
	typedef std::vector<HintList>::const_iterator const_iterator;
	const_iterator begin() const	{return m_hintar.begin();}
	const_iterator end() const	{return m_hintar.end();}

	const std::vector<HintList>& array() const
	{
		return m_hintar;
	}

private:
	std::vector<HintList> m_hintar;
};
