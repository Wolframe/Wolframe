...
// choose the key for the EventLog registry entry
	...
	if( ret != ERROR_SUCCESS ) {
		LOG_CRITICAL << "RegCreateKeyEx with key '" << key << "' failed: " << _Wolframe::Logging::Logger::LogWinerror;
		return false;
	}
...
// get service control manager
	...
	if( scm == NULL ) {
		LOG_CRITICAL << "OpenSCManager for service registration failed: " << _Wolframe::Logging::Logger::LogWinerror;
		return false;
	}
...
// create the service
	...
	if( service == NULL ) {
		LOG_CRITICAL << "CreateService during service registration failed: " << _Wolframe::Logging::Logger::LogWinerror;
		return false;
	}
...
// get service control manager
	SC_HANDLE scm = (SC_HANDLE)OpenSCManager( NULL, SERVICES_ACTIVE_DATABASE, SC_MANAGER_ALL_ACCESS );
	if( scm == NULL ) {
		LOG_CRITICAL << "OpenSCManager for service deregistration failed: " << _Wolframe::Logging::Logger::LogWinerror;
		return false;
	}
...
// get service handle of the service to delete (identified by service name)
	...
	if( service == NULL ) {
		LOG_CRITICAL << "OpenService during service deregistration failed: " << _Wolframe::Logging::Logger::LogWinerror;
		return false;
	}

// remove the service
	if( !DeleteService( service ) ) {
		LOG_CRITICAL << "Can't delete service: " << _Wolframe::Logging::Logger::LogWinerror;
		return false;
	}
...
// increase the checkpoint ticks for the progress bar in GUIs
	...
	if( !SetServiceStatus( serviceStatusHandle, &serviceStatus ) ) {
		LOG_FATAL << "Unable to report service state " << currentState << " to SCM: " << _Wolframe::Logging::Logger::LogWinerror;
		return;
	}
}
...
// register the event callback where we get called by Windows and the SCM
		...
		if( serviceStatusHandle == 0 ) {
			LOG_FATAL << "Unable to register service control handler function: " << _Wolframe::Logging::Logger::LogWinerror;
			return;
		}
...
// register a stop event
		...
		if( serviceStopEvent == NULL ) {
			LOG_FATAL << "Unable to create the stop event for the termination of the service: " << _Wolframe::Logging::Logger::LogWinerror;
...
// error, stop now immediatelly
				LOG_FATAL << "Waiting for stop event in service main failed, stopping now" << _Wolframe::Logging::Logger::LogWinerror;
				s.stop( );
				service_report_status( SERVICE_STOPPED, NO_ERROR, DEFAULT_SERVICE_TIMEOUT );
				return;
		}
		...
		if( cmdLineCfg.command == _Wolframe::Configuration::CmdLineConfig::RUN_SERVICE ) {
			...
			if( !StartServiceCtrlDispatcher( dispatch_table ) ) {
				if( GetLastError( ) == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT ) {
					...
					LOG_FATAL << "Unable to dispatch service control dispatcher: " << _Wolframe::Logging::Logger::LogWinerror;
					return _Wolframe::ErrorCodes::FAILURE;
				}
			} else {
