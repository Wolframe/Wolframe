...
bool ServerConfiguration::parse( const boost::property_tree::ptree& pt, const std::string& /* node */ )
{
	for ( boost::property_tree::ptree::const_iterator L1it = pt.begin(); L1it != pt.end(); L1it++ )	{
		if ( boost::algorithm::iequals( L1it->first, "threads" ))	{
			...
			if ( ! getUnsignedShortValue( L1it, displayName(), "maxConnections", maxConnections, os ))
				return false;
		}
			...
			for ( boost::property_tree::ptree::const_iterator L2it = L1it->second.begin();
				...
				if ( boost::algorithm::iequals( L2it->first, "host" ))	{
					if ( ! getHostnameValue( L2it, displayName(), "host", host, os ))
						return false;
				}
				else if ( boost::algorithm::iequals( L2it->first, "address" ))	{
					if ( ! getHostnameValue( L2it, displayName(), "address", host, os ))
						return false;
				}
				else if ( boost::algorithm::iequals( L2it->first, "port" ))	{
					if ( ! getUnsignedShortValue( L2it, displayName(), "port", port, os ))
						return false;
				}
				else if ( boost::algorithm::iequals( L2it->first, "maxConnections" ))	{
					if ( ! getUnsignedShortValue( L2it, displayName(), "maxConnections", maxConn, os ))
						return false;
				}
				else	{
			...
			for ( boost::property_tree::ptree::const_iterator L2it = L1it->second.begin();
				...
				if ( boost::algorithm::iequals( L2it->first, "host" ))	{
					...
					if ( ! getHostnameValue( L2it, displayName(), "certificate", certFile, os ))
						return false;
					...
					if ( ! getHostnameValue( L2it, displayName(), "key", keyFile, os ))
						return false;
					...
					if ( ! getHostnameValue( L2it, displayName(), "CAdirectory", CAdirectory, os ))
						return false;
					...
					if ( ! getHostnameValue( L2it, displayName(), "CAchainFile", CAchainFile, os ))
						return false;
					...
					if ( ! getBoolValue( L2it, displayName(), "verify", verify, os ))
						return false;
				}
				else	{
