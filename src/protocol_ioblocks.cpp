...
InputBlock::const_iterator InputBlock::getEoD( InputBlock::const_iterator start)
{
   unsigned int offset = start-const_cast<const InputBlock*>(this)->begin();
   if (size()<=offset) return const_cast<const InputBlock*>(this)->end();
   ...
   while (bufpos<bufsize)
   {
      ...
      else if (m_eodState == EoD::LF_DOT)
      {
         if (buf[bufpos] == '\r')
         {
            std::memmove( buf+dstsize++, "", 1);  //< feed EoD
   ...
   setPos( offset+dstsize);
   return const_cast<const InputBlock*>(this)->end();
}
