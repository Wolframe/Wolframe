...
struct inherit_visitors
{
    ...
    protected:
        ...
        {
            typedef T supported_type2;
            pEntry->first = typeid(supported_type2);
            ...
            pEntry->second = std::distance(
                ...
                reinterpret_cast< char* >(static_cast< type_visitor< supported_type2 >* >(this)));
...
private:
    ...
    void* get_visitor(std::type_info const& _type)
    {
        dispatching_map const& disp_map = get_dispatching_map();
        type_info_wrapper wrapper(_type);
