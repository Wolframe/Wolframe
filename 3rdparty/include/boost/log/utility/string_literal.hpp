...
public:
     ...
     * \return \c true if the comparand string equals to this string, \c false otherwise
     */
    bool operator== (const_pointer _str) const
    {
        return (compare_internal(m_pStart, m_Len, _str, traits_type::length(_str)) == 0);
    }
    /*!
     * Lexicographical comparison (equality)
     *
     ...
     * \b Throws: An <tt>std::exception</tt>-based exception if \a pos is out of range.
     */
    int compare(size_type pos, size_type n, const_pointer _str) const
    {
        return compare(pos, n, _str, traits_type::length(_str));
    }
    /*!
     ...
     *         a positive value if this string is greater than the comparand.
     */
    int compare(const_pointer _str, size_type len) const
    {
        return compare(0, m_Len, _str, len);
    }
    /*!
     * Lexicographically compares the argument string to this string
     *
    ...
    {
        return compare(0, m_Len, _str, traits_type::length(_str));
    }
    /*!
     * Lexicographically compares the argument string to this string
     *
