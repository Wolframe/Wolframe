...
public:
    ...
    size_type find(char_type c, size_type pos) const
    {
        ...
        if (pos < _size)
        {
            register const_pointer p = traits_type::find(m_pBegin + pos, _size - pos, c);
    ...
    size_type rfind(const_pointer that, size_type len, size_type pos) const
    {
        ...
        if (_size >= len)
        {
            register const_pointer pBegin = m_pBegin + (std::min)(pos, _size - len);
    ...
    size_type rfind(value_type c, size_type pos) const
    {
        ...
        if (_size > 0)
        {
            register const_pointer pBegin = m_pBegin + (std::min)(pos, _size - 1) + 1;
    ...
    size_type find_last_not_of(value_type c, size_type pos) const
    {
        ...
        if (_size > 0)
        {
            register const_pointer p = m_pBegin + (std::min)(pos, _size - 1);
    ...
    int compare(size_type pos1, size_type n1, const_pointer that, size_type len) const
    {
        ...
        if (pos1 <= _size)
        {
            const size_type part_size = (std::min)(n1, (size_type)_size - pos1);
...
//! Copy constructor
template< typename CharT, typename TraitsT >
basic_slim_string< CharT, TraitsT >::basic_slim_string(basic_slim_string const& that) : std::allocator< char >( ), m_pImpl(that.m_pImpl)
{
    m_pImpl->add_ref();
}
...
{
    const size_type _size = m_pImpl->size();
    if (pos <= _size)
    {
...
int basic_slim_string< CharT, TraitsT >::compare(size_type pos1, size_type n1, const_pointer s) const
{
    ...
    if (_size >= pos1)
    {
        register const int res = traits_type::compare(m_pImpl->begin() + pos1, s, (std::min)(n1, _size - pos1));
        if (res == 0)
            return (s[n1] == 0 ? 0 : -1);
        else
            return res;
