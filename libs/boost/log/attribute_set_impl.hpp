...
public:
    ...
    {
        typename node_list::const_iterator it = that.m_Nodes.begin(), _end = that.m_Nodes.end();
        for (; it != _end; ++it)
        {
    ...
    void erase(iterator it)
    {
        ...
        case 1: // The erased element is the last one in the bucket
            b.last = value_traits2::to_value_ptr(node_traits2::get_previous(b.last));
            break;

        case 2: // The erased element is the first one in the bucket
            b.first = value_traits2::to_value_ptr(node_traits2::get_next(b.first));
            break;
